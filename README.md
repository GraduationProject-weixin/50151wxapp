...
# 50151wxapp云匹面粉直供微信小程序+springboot
...
因此，开发合适的云匹面粉直供微信小程序，可以方便管理人员对云匹面粉直供微信小程序的管理，提高信息管理工作效率及查询效率，用户可以在手机上通过微信小程序来订餐，更加的方便，有利于更好的为人们服务。

1.2研究目的

随着互联网技术的快速发展，网络时代的到来，网络信息也将会改变当今社会。各行各业在日常企业经营管理等方面也在慢慢的向规范化和网络化趋势汇合。云匹面粉直供微信小程序的信息化程度体现在将互联网与信息技术应用于经营与管理，以现代化工具代替传统手工作业。无疑，使用网络信息化管理使信息管理更先进、更高效、更科学，信息交流更迅速。

对于之前云匹面粉直供微信小程序的管理，大部分都是使用传统的人工方式去管理，这样导致了管理效率低下、出错频率高。而且，时间一长的话，积累下来的数据信息不容易保存，对于查询、更新还有维护会带来不少问题。对于数据交接也存在很大的隐患。如果采用电子化的存储方式就会带来很大的改善，而且给用户的查询带来了很大便利，因此设计一个云匹面粉直供微信小程序刻不容缓，能够提高信息的管理水平。
## 1.3系统设计思想
...
# 3系统分析
## 3.1可行性分析
通过对本云匹面粉直供微信小程序实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本云匹面粉直供微信小程序后端采用SSM框架进行开发，是一种成熟的框架，开发有很多教程的，微信开发也有很多教程，并且网络上很多免费的教程。  

因此，云匹面粉直供微信小程序在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本云匹面粉直供微信小程序采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本云匹面粉直供微信小程序的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此云匹面粉直供微信小程序要严格控制管理权限，具体要求如下：

（1）要想对云匹面粉直供微信小程序进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
图4.14 面粉分类信息实体属性图
### 4.3.2数据库设计表
云匹面粉直供微信小程序需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|tablename|varchar(200)|是|yunpimianfen|商品表名|
...
|yuanliao|varchar(200)|否||原料|
|peiliao|varchar(200)|否||配料|
|zhongzhijidi|varchar(200)|是|NULL|种植基地|
|jinghanliang|varchar(200)|是|NULL|净含量|
|baozhiqi|varchar(200)|是|NULL|保质期|
|chucunfangfa|varchar(200)|是|NULL|储存方法|
|shiyong|varchar(200)|是|NULL|适用|
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|mianfenzhizuo|longtext|否||面粉制作|
|zhongzhijidi|varchar(200)|否||种植基地|
...
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|zhongzhijidi|varchar(200)|否||种植基地|
|caishouxianchang|longtext|否||采收现场|
|caishoutupian|varchar(200)|否||采收图片|
表4.13 种植基地
